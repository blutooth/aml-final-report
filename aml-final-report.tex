...
updates, at any given step. \todo{don't undertsnad}
...
from a slice-sampler \cite{neal2003slice}. Given this, we see that the number of likelihood evaluation  tends to oscillate up and down depending on the weight of its 'effective' step-size.
...
bright points, the expected number of which is given by the sum: \todo{PUT INFORMULA}.
...
\begin{table}[H]
  ...
  \end{tabular}
\end{table}
With the SSE from the table and the speedup observed from the plots of number
