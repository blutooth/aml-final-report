...
In this section we will need to discover whether the reductions in likelihood
evaluations per iteration that had been obtained in FlyMC would dominate the
increased burn-in period that is required when sampling using Firefly Monte
Carlo.
...
figures (c), where FlyMC and FlyMAP seem to return more modest reductions of aroud  1/3 and 1/6 of the likelihoods respectively.
...
To summarise, exactly same sorts of trends were found in our generated tests as were found in the paper by Ryan Adams \cite{maclaurin2014firefly}
