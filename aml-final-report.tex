...
For example, Monte Carlo Markov Chain (MCMC) are a class of recently resurging
\cite{diaconis2009markov} methods for sampling posterior distributions
$P(\theta | X)$ with intractable partition functions $Z = \int P(X|\theta)
P(\theta) d\theta$. When performing Metropolis-Hastings MCMC, at each iteration
we need to compute the acceptance probability
...
domains, scaling proven tools such as MCMC to support larger datasets would be
of great benefit to many different communities.
...
from a slice-sampler (which can adaptively choose for the size of the changes made)\footnote{http://people.ee.duke.edu/~lcarin/slice.pdf
}. Given this, we see that the number of likelihood evaluation  tends to oscillate up and down depending on the weight of its 'effective' step-size.
...
exactly the same sorts of trends that were observed in the paper by Ryan Adams \footnote{https://github.com/HIPS/firefly-monte-carlo%.

Crucially, however, the effectiveness of the firefly MC interpolation depends on the effective number of samples
per  next key quantity that ought to be observed is the
...
Table~\ref{tb:ess}. The results are based using, the corresponding
MCMC sampler.
